...
function M.run()
    ...
    assert(calls[1][1] == 160 and calls[1][2] == 90, "screen coordinates convert to game coordinates")
    ...
    delivered = scene_stack.screenpressed(stack, 500, 360, 1000, 720, 1, true, 1)
    assert(delivered == true, "touch-style pointer press is delivered")
    assert(#calls == 2 and calls[2][1] == 160 and calls[2][2] == 90, "touch coordinates use the same viewport conversion")
