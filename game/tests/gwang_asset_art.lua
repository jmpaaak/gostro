...
function M.run()
    ...
    local wealthy = assets.entry("gwang.wealthy_x2")
    assert(wealthy and wealthy.status == "runtime",
        "wealthy gwang must have tracked runtime artwork")
    assert(wealthy.master.width == 448 and wealthy.master.height == 256,
        "wealthy gwang must preserve its high-resolution slot master")
    assert(wealthy.runtime.width == 56 and wealthy.runtime.height == 32,
        "wealthy gwang runtime must scale exactly into a 28x16 slot")
    assert(wealthy.runtime.filter == "nearest")
    ...
    local boss_chips = assets.entry("gwang.boss_chips")
    assert(boss_chips and boss_chips.status == "runtime",
        "boss_chips gwang must have tracked runtime artwork")
    assert(boss_chips.master.width == 448 and boss_chips.master.height == 256,
        "boss_chips gwang must preserve its high-resolution slot master")
    assert(boss_chips.runtime.width == 56 and boss_chips.runtime.height == 32,
        "boss_chips gwang runtime must scale exactly into a 28x16 slot")
    assert(boss_chips.runtime.filter == "nearest")
    assert(assets.runtime_path("gwang.boss_chips") ==
        "assets/runtime/gwang/boss-chips-v1.png")

    local small_chips = assets.entry("gwang.small_chips")
    assert(small_chips and small_chips.status == "runtime",
        "small_chips gwang must have tracked runtime artwork")
    assert(small_chips.master.width == 448 and small_chips.master.height == 256,
        "small_chips gwang must preserve its high-resolution slot master")
    assert(small_chips.runtime.width == 56 and small_chips.runtime.height == 32,
        "small_chips gwang runtime must scale exactly into a 28x16 slot")
    assert(small_chips.runtime.filter == "nearest")
    assert(assets.runtime_path("gwang.small_chips") ==
        "assets/runtime/gwang/small-chips-v1.png")

    local big_mult = assets.entry("gwang.big_mult")
    assert(big_mult and big_mult.status == "runtime",
        "big_mult gwang must have tracked runtime artwork")
    assert(big_mult.master.width == 448 and big_mult.master.height == 256,
        "big_mult gwang must preserve its high-resolution slot master")
    assert(big_mult.runtime.width == 56 and big_mult.runtime.height == 32,
        "big_mult gwang runtime must scale exactly into a 28x16 slot")
    assert(big_mult.runtime.filter == "nearest")
