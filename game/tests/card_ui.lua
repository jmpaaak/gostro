...
function M.run()
    ...
    assert(card.LIFT == 8, "selected lift must be 8")
    ...
    assert(card.hit_test(h, 123, 135) == true, "bottom-right inside")
    assert(card.hit_test(h, 124, 136) == false, "outside right/bottom")
    assert(card.hit_test(h, 99, 100) == false, "outside left")
    -- hit_test uses draw_y for selected cards
    h.selected = true
    assert(card.hit_test(h, 100, 92) == true, "selected card lifted hitbox")
    assert(card.hit_test(h, 100, 91) == false, "above lifted hitbox")
