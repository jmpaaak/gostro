...
function M.run()
    ...
    assert(#first.gwang == 0 and #first.plaques.owned == 0 and #first.seals.owned == 0)
    assert(first.plaques == first.tags, "legacy tag state aliases canonical plaques")
    assert(first.vouchers == first.seals, "legacy save field aliases canonical 인장 state")
    ...
    first.seals.owned[1] = "market_bundle"
    assert(#second.plaques.owned == 0 and #second.seals.owned == 0,
