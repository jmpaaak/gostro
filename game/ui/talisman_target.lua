-- game/ui/talisman_target.lua
-- Headless-safe talisman option and single-card target selection. This module
-- builds a use request but deliberately never calls game.talismans.use.
...
local function definition(flow)
    ...
    if not def then
        error("unknown talisman target effect: " .. tostring(flow and flow.effect))
    end
    return def
end
...
function M.new(slot, talisman)
    assert(type(slot) == "number" and slot >= 1 and slot % 1 == 0,
        "talisman target requires a positive slot")
    assert(type(talisman) == "table", "talisman target requires a talisman")
    if not EFFECTS[talisman.effect] then
        error("unknown talisman target effect: " .. tostring(talisman.effect))
    end
    return {
        slot = slot,
        talisman_id = talisman.id,
        talisman_name = talisman.name,
        -- Legacy field aliases for callers holding an old modal snapshot.
        tarot_id = talisman.id,
        tarot_name = talisman.name,
...
function M.view(flow, cards)
    assert(type(flow) == "table", "talisman target UI requires flow state")
    assert(type(cards) == "table", "talisman target UI requires target cards")
    ...
    return {
        ...
        cancel_label = "취소",
        title = flow.talisman_name
            and (flow.talisman_name .. " · " .. def.title) or def.title,
...
--- Produce arguments for talismans.use only when the selection is complete.
