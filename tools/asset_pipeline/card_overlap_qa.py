...
def main():
    ...
    if (sheet_width, sheet_height) != (120, 36):
        raise ValueError("contact sheet must be the 120x36 Pixel Perfect output")
    cards = [crop(pixels, sheet_width, index * 24, 0, 24, 36) for index in range(5)]
    fingerprints = [fingerprint(card, 24) for card in cards]
    if len(set(fingerprints)) != 5:
        raise ValueError("top-edge card marks are not uniquely identifiable")
    step, margin = 10, 2
    capture_width, capture_height = margin * 2 + 24 + step * 4, 40
    canvas = bytearray(capture_width * capture_height * 4)
    for index, card in enumerate(cards):
        paste(canvas, capture_width, card, 24, 36, margin + index * step, 2)
