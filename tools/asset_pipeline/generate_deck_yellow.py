#!/usr/bin/env python3
"""Project-authored 384x384 yellow hwatu deck thumbnail master."""
from math import cos, pi, sin
from pathlib import Path
from PIL import Image, ImageDraw

W, H, OUT = 384, 384, Path("assets/masters/ui/deck-yellow-v1.png")
...
def run():
    ...
    d.rounded_rectangle((8, 16, W - 8, H - 4), 36, fill=(18, 12, 8, 255))
    d.rounded_rectangle((8, 8, W - 8, H - 16), 36, fill=(168, 124, 32, 255))
    d.rounded_rectangle((24, 24, W - 24, H - 32), 28, fill=(112, 72, 18, 255))
    d.rounded_rectangle((40, 40, W - 40, H - 48), 22, outline=GOLD, width=10)
    d.rounded_rectangle((56, 56, W - 56, H - 64), 16, outline=(40, 24, 10, 255), width=6)
    for i in range(8):
        a = i * pi / 4 + 0.2
        d.polygon([polar(cx, cy - 8, 18, a), polar(cx, cy - 8, 92, a - 0.08), polar(cx, cy - 8, 92, a + 0.08)],
                  fill=(214, 176, 74, 90) if i % 2 == 0 else (168, 42, 48, 70))
    d.ellipse((cx - 54, cy - 62, cx + 54, cy + 46), fill=(186, 142, 48, 255))
    d.ellipse((cx - 40, cy - 48, cx + 40, cy + 32), fill=(240, 220, 170, 255))
    star(d, cx, cy - 8, 34, 14, 8, GOLD, (255, 232, 150, 255))
    star(d, cx, cy - 8, 14, 5, 8, (40, 24, 10, 255))
    d.polygon([(cx - 92, cy - 18), (cx - 48, cy - 48), (cx - 52, cy + 8)], fill=CRIMSON, outline=GOLD)
    d.rectangle((cx - 98, cy - 54, cx - 90, cy + 12), fill=(18, 12, 8, 255))
    d.polygon([(cx + 92, cy - 18), (cx + 48, cy - 48), (cx + 52, cy + 8)], fill=INDIGO, outline=GOLD)
    d.rectangle((cx + 90, cy - 54, cx + 98, cy + 12), fill=(18, 12, 8, 255))
