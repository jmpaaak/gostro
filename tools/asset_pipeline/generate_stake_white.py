#!/usr/bin/env python3
"""Project-authored 384x384 white hwatu stake chip master."""
from math import cos, pi, sin
from pathlib import Path
from PIL import Image, ImageDraw

W, H, OUT = 384, 384, Path("assets/masters/ui/stake-white-v1.png")
IVORY = (236, 228, 206, 255)
PEARL = (248, 242, 224, 255)
GOLD = (196, 158, 72, 255)
CRIMSON = (168, 42, 48, 255)
INDIGO = (42, 78, 148, 255)
LACQUER = (28, 22, 16, 255)
...
def run():
    ...
    d.ellipse((8, 12, W - 8, H - 4), fill=LACQUER)
    d.ellipse((8, 8, W - 8, H - 12), fill=IVORY)
    d.ellipse((28, 28, W - 28, H - 32), fill=(214, 204, 178, 255))
    d.ellipse((48, 48, W - 48, H - 52), outline=GOLD, width=10)
    d.ellipse((68, 68, W - 68, H - 72), outline=(92, 68, 28, 255), width=6)
    for i in range(8):
        a = i * pi / 4 + 0.18
        d.polygon(
            [polar(cx, cy - 4, 22, a), polar(cx, cy - 4, 118, a - 0.07), polar(cx, cy - 4, 118, a + 0.07)],
            fill=(196, 158, 72, 70) if i % 2 == 0 else (168, 42, 48, 55),
        )
    d.ellipse((cx - 58, cy - 62, cx + 58, cy + 50), fill=PEARL)
    d.ellipse((cx - 40, cy - 44, cx + 40, cy + 32), fill=(255, 250, 236, 255))
    star(d, cx, cy - 6, 36, 14, 8, GOLD, (255, 232, 150, 255))
    star(d, cx, cy - 6, 14, 5, 8, LACQUER)
    d.polygon([(cx - 108, cy - 8), (cx - 62, cy - 42), (cx - 68, cy + 16)], fill=CRIMSON, outline=GOLD)
    d.rectangle((cx - 116, cy - 46, cx - 106, cy + 20), fill=LACQUER)
    d.polygon([(cx + 108, cy - 8), (cx + 62, cy - 42), (cx + 68, cy + 16)], fill=INDIGO, outline=GOLD)
    d.rectangle((cx + 106, cy - 46, cx + 116, cy + 20), fill=LACQUER)
    d.polygon(
