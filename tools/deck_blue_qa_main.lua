...
function love.load()
    ...
    assert(run_setup.selected_deck(state).id == "hwatu", "QA capture must show the default blue deck")
    ...
    local output = assert(os.getenv("DECK_BLUE_QA_OUTPUT"), "DECK_BLUE_QA_OUTPUT is required")
    write_capture(canvas, output)
    print("DECK_BLUE_LOVE_QA_OK 320x180 " .. output)
    love.event.quit(0)
end
