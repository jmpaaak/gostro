...
6. Run `make test LOVE=/Users/jm/.local/bin/love` before a checkpoint commit. Do **not** run `make verify` in the autonomous loop — it launches windowed capture QA and shows GostroLoop. Never launch `/Users/jm/.local/bin/love` or `love` directly. Loop children inherit `GAME_HEADLESS=1` and dummy SDL.
